...
```bash
pytest         # 132 tests: core (incl. the O(1) BookStore symbol index + MarketState matrix cache + bounded multi-leg graph search), all 15 strategies, execution/risk (incl. the pre-execution profitability re-check), dashboard API + Basic Auth + the /ws ticket fallback + venue credentials, config venue-id validation, hypothesis property tests -- no live network calls
ruff check .   # clean
