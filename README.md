...
6. **Rate limits and quirks across 15+ CEX and 8+ DEX venues will need real hardening.** ccxt smooths over a lot; expect exchange-specific edge cases in production that this reference engine's REST fallback (see point 8) does not fully paper over.
...
```bash
pytest         # 137 tests: core (incl. the O(1) BookStore symbol index + MarketState matrix cache + bounded multi-leg graph search), all 15 strategies, execution/risk (incl. the pre-execution profitability re-check), analytics (incl. bounded in-memory history), dashboard API + Basic Auth + the /ws ticket fallback + venue credentials, config venue-id validation, hypothesis property tests -- no live network calls
ruff check .   # clean
